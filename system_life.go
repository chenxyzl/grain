...
func (x *system) Start() {
	...
	//register to cluster
	if err := x.clusterProvider.start(x, x.clusterMemberChanged, x.addr(), x.config, x.logger); err != nil {
...
func (x *system) init(nodeId uint64) {
	...
	//overwrite logger
	x.logger = slog.With("system", x.addr(), "node", x.config.state.NodeId)
...
func (x *system) stopActors() {
	...
	for {
		...
		x.registry.lookup.IterCb(func(key string, v iProcess) {
			//ignore poison
			if v.self().GetDirectAddr() != x.addr() ||
